"""Generate test data for the Secrets Providers app."""
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--flush",
            action="store_true",
            help="Flush any existing Secrets Providers test data from the database before generating new data.",
        )

    def _generate_static_data(self, db):
        """Generate static data required for test cases."""
        # TODO: populate the database with Secrets Providers test data.

    def handle(self, *args, **options):
        """Entry point to the management command."""
        if options["flush"]:
            self.stdout.write("Flushing existing Secrets Providers test data...")
