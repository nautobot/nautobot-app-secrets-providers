...
def _is_compose_included(context, name):
    return (
        f"docker-compose.{name}.yml" in context.nautobot_secrets_providers.compose_files
    )
...
def docker_compose(context, command, **kwargs):
    ...
    for compose_file in context.nautobot_secrets_providers.compose_files:
        compose_file_path = os.path.join(
            context.nautobot_secrets_providers.compose_dir, compose_file
        )
...
def build(context, force_rm=False, cache=True):
    ...
    print(
        f"Building Nautobot with Python {context.nautobot_secrets_providers.python_ver}..."
    )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    dockerfile_path = os.path.join(
        context.nautobot_secrets_providers.compose_dir, "Dockerfile"
    )
    base_image = (
...
def lock(
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.nautobot_secrets_providers.python_ver}"
    else:
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_secrets_providers/*' --omit *migrations*"
